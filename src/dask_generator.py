  ...
  for cword, tword in zip(camera_names, logs_names):
    try:
      with h5py.File(tword, "r") as t5:
        ...
        #print len(angle), "angle values"

        goods = np.abs(angle[-1]) <= 200

        #print len(goods), "goods values"

        filters.append(np.argwhere(goods)[time_len-1:] + (lastidx+time_len-1))

        #print len(filters), "filters values"
...
def datagen(filter_files, time_len=1, batch_size=256, ignore_goods=False, show_time=True):
  """
  Parameters:
  -----------
